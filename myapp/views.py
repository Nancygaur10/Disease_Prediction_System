...
path=os.path.dirname(__file__) # provides root path of myapp
...
def prediction(req):
    if req.method=='POST':
        ...
        his.save()  # insert into history values(...)
        return render(req,"prediction_result.html",{"res":res})
    
    return render(req,"prediction.html")
def history(req):
    his = History.objects.all().order_by('-id')  #select * from history
    paginator = Paginator(his, 3)

    page_number = req.GET.get('page')
    page_obj = paginator.get_page(page_number)

    return render(req,"history.html",{"page_obj":page_obj})
def prediction_result(req):
    return render(req,"prediction_result.html")